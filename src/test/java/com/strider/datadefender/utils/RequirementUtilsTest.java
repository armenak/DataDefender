...
public class RequirementUtilsTest extends TestCase {
    ...
    public void testCreate() { // just test the important stuff for now (as we aren't sure if the defaults make sense)
        List<MatchMetaData> list = new ArrayList<MatchMetaData>() {{  
            add(new MatchMetaData(null, "t1", Arrays.asList("pk"), "col1", "String", 5)); // one pk, 1 column
            add(new MatchMetaData(null, "t2", Arrays.asList("pk1", "pk2"), "col1", "String", 5)); // two pk, two columns
            add(new MatchMetaData(null, "t2", Arrays.asList("pk1", "pk2"), "col2", "String", 5));
        }};
